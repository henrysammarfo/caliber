...
const slim = hits.map(it => ({
  ...
  min_price_usdc: it.min_price_usdc, input_schema: it.input_schema, output_schema: it.output_schema,
...
for (const s of slim) {
  ...
  console.log([s.id,s.slug,s.activation_status,s.scored,intents,rank,s.yaml_url].join('\t'));
}
